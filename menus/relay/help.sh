...
menu_manage_relay_help ()
{
    ...
    text_white "    stops the DinPay relay node process (shown only if the relay node is running)."
    echo

    text_yellow "    Start Relay (S)"
    text_white "    starts the DinPay relay node process (shown only if the relay node is not running)."
    echo
