...
apt_package_installed()
{
    ...
    install_status=$(dpkg --list "$package" 2>>"$launcher_log" | tail -n 1 | head -c 3) || true
...
install_base_dependencies ()
{
    ...
    sudo apt-get install -y git curl | tee -a "$launcher_log"
...
install_program_dependencies ()
{
    ...
    if [[ ! -z "$TO_INSTALL" ]]; then
        ...
        sudo sh -c "sudo apt-get install $TO_INSTALL -y" | tee -a "$launcher_log"
...
install_nodejs_dependencies ()
{
    ...
    if [[ ! -z "$TO_INSTALL" ]]; then
        ...
        sh -c "sudo npm install -g $TO_INSTALL" | tee -a "$launcher_log"
...
install_system_updates ()
{
    ...
    sudo apt-get update | tee -a "$launcher_log"
    sudo apt-get upgrade -yqq | tee -a "$launcher_log"
    sudo apt-get dist-upgrade -yq | tee -a "$launcher_log"
    sudo apt-get autoremove -yyq | tee -a "$launcher_log"
    sudo apt-get autoclean -yq | tee -a "$launcher_log"
